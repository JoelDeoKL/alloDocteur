...
class SignalController extends AbstractController
{
    ...
    {
        $signals = $entityManager->getRepository(Signal::class)->findAll();
    ...
    public function editer_signal(Signal $signal = null, EntityManagerInterface $entityManager, ManagerRegistry $doctrine, Request $request, UserPasswordHasherInterface $userPasswordHasher): Response
    {
        $new = false;
        if(!$signal){
            $new = true;
    ...
    public function detail_signal(ManagerRegistry $doctrine, Signal $signal= null, $id): Response
    {
        if(!$signal){
    ...
    public function signal_detail(ManagerRegistry $doctrine, Signal $signal= null, $id): Response
    {
        if(!$signal){
            $this->addFlash('error', "Ce signal n'existe pas !");
            return $this->redirectToRoute("signals");
    ...
    public function delete_signal(Signal $signal = null, ManagerRegistry $doctrine, Request $request, $id): Response
    {

        $repository = $doctrine->getRepository(Signal::class);
